...
public class ProductController {
	...
	public String insertProduct(@ModelAttribute("product") Product product,
		...
		String imagepath = "C:\\Users\\82103\\eclipse-workspace\\ShopifyFrontend\\src\\main\\webapp\\resources\\images\\";
	...
	@RequestMapping(value = "/productDisplay/{categoryId}")
	public String displayProductsByCategory(@PathVariable("categoryId") int categoryId, Model m) {
		m.addAttribute("pageinfo", "Product Catalog-");

		// categoryId에 해당하는 카테고리에 속한 제품을 가져오도록 수정
	...
	public String displayAllProducts(Model m) {
		m.addAttribute("pageinfo", "All Products");

		List<Product> listProducts = productDAO.listProducts();
		m.addAttribute("productList", listProducts);

		return "ProductDisplay"; // 또는 실제로 사용하는 뷰 이름으로 변경
	}
